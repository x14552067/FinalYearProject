...
(function() {
  var activate_interactions, addLecturerMessage, addQuestionMessage, addStudentMessage;

  addLecturerMessage = function(payload) {
    var message_content, messages, name, timestamp;
  ...
  addStudentMessage = function(payload) {
    var message_content, messages, name, timestamp;
    message_content = payload['content'];
    timestamp = payload['timestamp'];
    name = payload['name'];
  ...
  addQuestionMessage = function(payload) {
    var message_content, messages, name, timestamp;
    message_content = payload['content'];
    timestamp = payload['timestamp'];
    name = payload['name'];
  ...
  App.session = App.cable.subscriptions.create("SessionChannel", {
    ...
    received: function(data) {
      ...
      if (payload['type'] === 'chat') {
        if (payload['utp'] === "191") {
          return addLecturerMessage(payload);
        } else {
          return addStudentMessage(payload);
        }
      } else if (payload['type'] === 'question') {
        return addQuestionMessage(payload);
      }
    },
    send_message: function(type) {
      var anon, content, payload;
      if (type === "c") {
        ...
        return this.perform('send_question_message', {
          message: payload
        });
      } else if (type = "a") {
  ...
  }, $(document).on('keypress', '[data-behavior~=chat_send]', function(event) {
    if (event.keyCode === 13) {
      App.session.send_message("c");
      return event.preventDefault();
    }
  }), $(document).on('keypress', '[data-behavior~=question_send]', function(event) {
    if (event.keyCode === 13) {
      App.session.send_message("q");
      return event.preventDefault();
    }
  }), activate_interactions = function() {
    var messages;
    messages = document.getElementById('chat-messages');
    ...
    return $("#send-question").on('click', function(event) {
      App.session.send_message("q");
      return event.preventDefault();
    });
